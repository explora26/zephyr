...
static int erase_sector(struct device *dev, uint32_t sector)
{
	...
	FLASH->CR1 |= (FLASH_CR_SER | FLASH_PSIZE_WORD | (sector << FLASH_CR_SNB_Pos) | FLASH_CR_START);

	regs->CR1 = (regs->CR1 & (CR_PSIZE_MASK | STM32F7X_SECTOR_MASK)) |
		   FLASH_PSIZE_BYTE |
		   FLASH_CR_SER |
...
#if defined (DUAL_BANK)
	if (FLASH_STM32_REGS(dev)->SR2 & error2) {
		LOG_DBG("Status: 0x%08x", FLASH_STM32_REGS(dev)->SR2 & error);
		return -EIO;
	}
#endif
